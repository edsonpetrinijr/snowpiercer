...
export default function DialogueBox({ kid, onClose }: Props) {
  const [step, setStep] = useState<"kid" | "curtis">("kid");
  ...
  return (
    ...
    >
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/60" onClick={step === "kid" ? () => setStep("curtis") : onClose} />
        ...
        {step === "kid" ? (
          ...
          >
            ...
            <p className="text-zinc-200 text-base leading-relaxed">
              &ldquo;{kid.dialogue}&rdquo;
            </p>

            {/* Continue button */}
            <button
              onClick={() => setStep("curtis")}
          ...
          <motion.div
            key="curtis"
            className="relative w-full max-w-2xl bg-zinc-900 border border-amber-500/30 rounded-2xl p-6 shadow-2xl"
            initial={{ y: 40, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            exit={{ y: 40, opacity: 0 }}
            transition={{ duration: 0.3 }}
          >
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="font-semibold text-amber-400">Curtis</p>
                <p className="text-xs text-zinc-500">Revolutionary · Car 001</p>
              </div>
              <button
                onClick={onClose}
